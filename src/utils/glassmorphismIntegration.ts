...
export const applyGlassmorphismTheme = (
  element: HTMLElement,
  variant: 'card' | 'button' | 'input' = 'card'
): void => {
  const baseClasses = {
  ...
  element.className = combineGlassClasses(
    element.className,
    baseClasses[variant]
  )
}
...
export const injectGlassmorphismCSSProperties = (
  ...
  Object.entries(theme.colors.text).forEach(([key, value]) => {
    root.style.setProperty(
      `--glass-text-${key.replace(/([A-Z])/g, '-$1').toLowerCase()}`,
      value
    )
  })

  // Background and surface colors
  root.style.setProperty('--glass-background', theme.colors.background.main)
...
export const initializeGlassmorphismSystem = (
  theme?: GlassmorphismTheme
): void => {
