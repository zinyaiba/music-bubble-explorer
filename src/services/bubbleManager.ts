...
} from './advancedAnimationController'
import {
  DEFAULT_BUBBLE_SETTINGS,
  type BubbleSettings,
  getCurrentBubbleSettings,
} from '@/config/bubbleSettings'
...
export class BubbleManager {
  ...
  constructor(
    ...
    config: BubbleConfig,
    advancedAnimationConfig?: Partial<AdvancedAnimationConfig>
  ) {
  ...
  generateBubble(): BubbleEntity {
    ...
    while (attempts < maxAttempts) {
      ...
      if (selectedType === 'song') {
        ...
        if (!tag) {
          ...
          if (!song) {
            throw new Error(
              'Cannot generate bubble: No songs or tags available'
            )
          }
          name = song.title
          relatedCount =
        ...
        if (!person) {
          throw new Error(
            `Cannot generate bubble: No ${selectedType} available`
          )
        }
        name = person.name
    ...
    console.warn(
      'Could not generate unique bubble after maximum attempts, allowing duplicate'
    )
    ...
    bubble.size = this.calculateBubbleSize(
      song.lyricists.length + song.composers.length + song.arrangers.length
    )
    bubble.color =
      LOVELY_COLORS[Math.floor(Math.random() * LOVELY_COLORS.length)]
  ...
  calculateBubbleSize(relatedCount: number): number {
    ...
    if (this.config.minSize === this.config.maxSize) {
      console.log(
        `🫧 Fixed size mode: returning ${this.config.minSize}px (min=${this.config.minSize}, max=${this.config.maxSize})`
      )
    ...
    const calculatedSize =
      this.config.minSize +
      (this.config.maxSize - this.config.minSize) * normalizedCount
    ...
    if (import.meta.env.DEV && Math.random() < 0.1) {
      // 10%の確率でのみログ出力
      console.log(
        `🫧 Size calculation: relatedCount=${relatedCount}, normalized=${normalizedCount.toFixed(2)}, calculated=${calculatedSize.toFixed(1)}, final=${finalSize.toFixed(1)} (min=${this.config.minSize}, max=${this.config.maxSize})`
      )
    }
  ...
  updateBubblePhysics(bubbles: BubbleEntity[]): BubbleEntity[] {
    ...
    const aliveBubbles = bubbles.filter(
      bubble => bubble.isAlive() && !bubble.isMarkedForDeletion()
    )
    ...
    return aliveBubbles
      .map(bubble => {
        ...
        const shouldDisappear =
          this.animationManager.shouldStartDisappearAnimation(
            updatedBubble.getAge(),
            updatedBubble.lifespan + updatedBubble.getAge()
          )
        const currentAnimation = this.animationManager.getAnimationState(
          updatedBubble.id
        )

        if (
          shouldDisappear &&
          (!currentAnimation || currentAnimation.type !== 'disappear')
        ) {
          this.animationManager.startDisappearAnimation(updatedBubble.id)
        }

        // 改善された浮遊物理（パーリンノイズ、ベジェ曲線）
        this.applyImprovedFloatingPhysics(
          updatedBubble,
          clampedDeltaTime,
          currentTime
        )
   ...
   */
  private applyImprovedFloatingPhysics(
    bubble: BubbleEntity,
    deltaTime: number,
    currentTime: number
  ): void {
    ...
    const noiseOffset = this.animationManager.getNoiseOffset(
      bubble.id,
      currentTime
    )
    ...
    // 複数のベジェ曲線軌道を組み合わせ
    const naturalTrajectory = this.calculateNaturalTrajectory(
      timeInSeconds,
      trajectoryAmplitude,
      bubble.size
    )
    const buoyancyTrajectory = this.calculateBuoyancyTrajectory(
      timeInSeconds,
      bubble.size
    )
    const windTrajectory = this.calculateWindTrajectory(
      timeInSeconds,
      sizeRatio
    )
    ...
    // 軌道を合成
    bubble.vx +=
      (naturalTrajectory.x + buoyancyTrajectory.x + windTrajectory.x) *
      deltaTime
    bubble.vy +=
      (naturalTrajectory.y + buoyancyTrajectory.y + windTrajectory.y) *
      deltaTime
    ...
    if (!currentAnimation || currentAnimation.type === 'floating') {
      ...
      const breathingFrequency =
        this.config.breathingFrequency + bubble.size / 300
      const breathingAmplitude =
        this.config.breathingAmplitude + sizeRatio * 0.02
      ...
      const breathingEffect =
        Math.sin(timeInSeconds * breathingFrequency) * breathingAmplitude
   ...
   */
  private calculateNaturalTrajectory(
    time: number,
    amplitude: number,
    bubbleSize: number
  ): { x: number; y: number } {
    ...
    const secondaryY = this.calculateBezierMovement(
      t * 1.7 + 0.5,
      amplitude * 0.2
    )
   ...
   */
  private calculateBuoyancyTrajectory(
    time: number,
    bubbleSize: number
  ): { x: number; y: number } {
    const buoyancyFactor =
      Math.max(0.2, 1 - bubbleSize / 150) * (this.config.buoyancyStrength / 15) // 設定ファイル対応
    const t = (time * 0.2) % 1
   ...
   */
  private calculateWindTrajectory(
    time: number,
    sizeRatio: number
  ): { x: number; y: number } {
  ...
  private getRandomPerson(
    ...
    const people = this.musicDatabase.people.filter(
  ...
  private updateAnimations(bubbles: BubbleEntity[], currentTime: number): void {
    bubbles.forEach(bubble => {
      // アニメーションスケールと透明度を更新
      const scale = this.animationManager.getCurrentScale(
        bubble.id,
        currentTime
      )
      const opacity = this.animationManager.getCurrentOpacity(
        bubble.id,
        currentTime,
        bubble.opacity
      )
      ...
      const rotation = this.animationManager.getCurrentRotation(
        bubble.id,
        currentTime
      )
      if (rotation !== 0) {
    ...
    // 完了したアニメーションをクリーンアップ
    const completedDisappearAnimations =
      this.animationManager.cleanupCompletedAnimations(currentTime)
    ...
    try {
      ...
      if (this.bubbles.length > this.config.maxBubbles) {
        ...
        if (import.meta.env.DEV && Math.random() < 0.2) {
          console.log(
            `🫧 Bubble count exceeded limit: ${this.bubbles.length} > ${this.config.maxBubbles}, removing excess bubbles`
          )
        }
        // 古いシャボン玉から削除
      ...
      console.error(
        'Critical error in updateFrame (returning current state):',
        error
      )
      return [...this.bubbles]
    }
  ...
  getStats() {
    ...
    return {
      ...
      averageSize:
        this.bubbles.reduce((sum, b) => sum + b.size, 0) /
          this.bubbles.length || 0,
      averageLifespan:
        this.bubbles.reduce((sum, b) => sum + b.lifespan, 0) /
          this.bubbles.length || 0,
      tagStats: tagStats,
    }
  }
  ...
  createNaturalDisappearancePattern(): void {
    if (this.advancedAnimationController) {
      this.advancedAnimationController.createNaturalDisappearancePattern(
        this.bubbles
      )
    }
  }
  ...
  calculateStaggeredDisappearanceTiming(): number[] {
    ...
    return this.advancedAnimationController.calculateStaggeredTiming(
      this.bubbles.length
    )
  }
 ...
 * 設定ファイルから値を取得
 */
export function createBubbleConfig(
  canvasWidth: number,
  canvasHeight: number
): BubbleConfig {
