...
export const useTagList = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const finalTags = serviceTags.length > musicDatabase.tags.length ? serviceTags : musicDatabase.tags
      ...
      if (finalTags.length === 0) {
        ...
        if (finalSongs.length === 0) {
          if (!firebaseConnection.isConnected) {
            setError('Firebaseに接続できません。ネットワーク接続を確認してください。')
          } else {
            setError('楽曲データが登録されていません。まず楽曲を登録してください。')
          }
        } else {
          const songsWithTags = finalSongs.filter(song => song.tags && song.tags.length > 0)
          console.log('🏷️ Songs with tags:', {
            totalSongs: loadedSongs.length,
            songsWithTags: songsWithTags.length,
            songsWithTagsDetails: songsWithTags.map(s => ({ title: s.title, tags: s.tags }))
          })
          
          if (songsWithTags.length === 0) {
            setError('タグが設定された楽曲がありません。楽曲にタグを追加してください。')
          } else {
            // 手動でタグを抽出してみる
            ...
            if (manualTags.size > 0) {
              ...
              const manualTagArray = Array.from(manualTags).map(tagName => ({
                id: `tag-${tagName}`,
                name: tagName,
                songs: finalSongs.filter(song => song.tags && song.tags.includes(tagName)).map(song => song.id)
              }))
              
              console.log('🏷️ Using manually extracted tags:', manualTagArray)
              setTags(manualTagArray)
            } else {
              setError('タグデータの処理中にエラーが発生しました。楽曲にタグは設定されていますが、タグ一覧の生成に失敗しました。')
            }
          }
        }
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'タグデータの読み込みに失敗しました'
  ...
  const tagListItems = useMemo((): TagListItem[] => {
    ...
    return tags.map(tag => {
      const tagSongs = songs.filter(song => 
        song.tags && song.tags.includes(tag.name)
      )
  ...
  const getSongsForTag = useCallback((tagName: string): Song[] => {
    if (!Array.isArray(songs)) {
      return []
    }
    return songs.filter(song => 
      song.tags && song.tags.includes(tagName)
    )
  }, [songs])
  ...
  const getTagStatistics = useCallback(() => {
    ...
    const averageTagsPerSong = totalSongs > 0 && Array.isArray(songs)
      ? songs.reduce((sum, song) => sum + (song.tags?.length || 0), 0) / totalSongs 
      : 0
    
    const mostPopularTag = tagListItems.reduce((prev, current) => 
      (prev.songCount > current.songCount) ? prev : current, 
      tagListItems[0]
    )
