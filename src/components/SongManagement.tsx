    ...
    const handleConfirmDelete = useCallback(async () => {
        ...
        try {
            ...
            if (!localDeleteSuccess) {
                ...
                const detailedError = DataManager.getDetailedErrorMessage(new Error('Delete operation failed'))
                                ...
                                <div className="song-details">
                                    {song.lyricists.length > 0 && (
                                        <div className="detail-item">
                                            ...
                                            <span className="detail-value">{song.lyricists.join(', ')}</span>
                                        </div>
                                    )}
                                    {song.composers.length > 0 && (
                                        <div className="detail-item">
                                            <span className="detail-label">作曲:</span>
                                            <span className="detail-value">{song.composers.join(', ')}</span>
                                        </div>
                                    )}
                                    {song.arrangers.length > 0 && (
                                        <div className="detail-item">
                                            <span className="detail-label">編曲:</span>
                                            <span className="detail-value">{song.arrangers.join(', ')}</span>
                                        </div>
                                    )}
                                    {song.tags && song.tags.length > 0 && (
                                        <div className="detail-item">
                                            <span className="detail-label">タグ:</span>
                                            <span className="detail-value">{song.tags.join(', ')}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
