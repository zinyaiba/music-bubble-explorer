...
export const TagInput: React.FC<TagInputProps> = React.memo(
  ({
    ...
    const generateSuggestions = useCallback(
      (input: string): TagSuggestion[] => {
        if (!input.trim()) return []

        const inputLower = input.toLowerCase()
        const safeExistingTags = existingTags || []

        // 既に追加済みのタグを除外（大文字小文字を区別しない）
        const addedTagsLower = tags.map(tag => tag.toLowerCase())
        ...
        const filteredExisting = safeExistingTags
          .filter(tag => {
            ...
            return (
              ...
              !addedTagsLower.includes(tagLower)
            )
          })
          .map(tag => ({ tag, isExisting: true }))

        // 入力値が既存タグと完全一致しない場合、新規タグとして提案
        const exactMatch = safeExistingTags.some(
          tag => tag.toLowerCase() === inputLower
        )
        const alreadyAdded = addedTagsLower.includes(inputLower)
    ...
    return (
      <TagInputContainer onClick={handleContainerClick} $disabled={disabled}>
        ...
        {tags.length >= maxTags && (
          <MaxTagsMessage>最大{maxTags}個のタグまで追加できます</MaxTagsMessage>
        )}
      </TagInputContainer>
    )
  }
...
const TagInputContainer = styled.div<{ $disabled?: boolean }>`
  ...
  border-radius: 12px;
  background: ${({ $disabled }) =>
    $disabled ? 'rgba(240, 240, 240, 0.8)' : 'rgba(255, 255, 255, 0.8)'};
...
const TagChip = styled.div<{ $disabled?: boolean }>`
  ...
  &:hover {
    background: ${({ $disabled }) =>
      $disabled ? undefined : 'linear-gradient(135deg, #90ee90, #7dd87d)'};
...
const SuggestionItem = styled.div<{
  ...
  background: ${({ $isSelected }) =>
    $isSelected ? 'rgba(255, 105, 180, 0.1)' : 'transparent'};
...
const SuggestionBadge = styled.span<{ $isExisting: boolean }>`
  ...
  border: 1px solid
    ${({ $isExisting }) => ($isExisting ? '#90ee90' : '#ff69b4')};
`
